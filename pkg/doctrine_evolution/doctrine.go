...
func New(cfg *config.ModelConfig) *DoctrineAnalyzer {
	return &DoctrineAnalyzer{cfg: cfg}
}
...
func (a *DoctrineAnalyzer) detectChangePoints(
	series []struct {
		Year   int
		Value  float64
		Weight float64
...
func (a *DoctrineAnalyzer) ComputeChangePoints(
	...
	yearSeries := make([]struct {
		Year   int
		Value  float64
		Weight float64
	}, 0, len(bfs))
	for _, bf := range bfs {
		yearSeries = append(yearSeries, struct {
			Year   int
			Value  float64
			Weight float64
		}{bf.BattleYear, bf.Elevation, float64(bf.TotalTroops)})
	}
